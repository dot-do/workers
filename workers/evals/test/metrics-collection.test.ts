...
/**
 * Interface definition for EvalsDO metrics operations
 */
interface EvalsDOMetricsContract {
  // Metrics collection
  collectMetric(runId: string, metric: MetricInput): Promise<Metric>
  getMetrics(runId: string, options?: MetricQueryOptions): Promise<Metric[]>
  aggregateMetrics(runId: string, options?: AggregateOptions): Promise<AggregatedMetrics>

  // Model metrics
  getModelMetrics(model: string, options?: ModelMetricOptions): Promise<ModelAggregatedMetrics>
  compareModels(models: string[], metric: string, options?: CompareOptions): Promise<ModelComparison>

  // Time series
  getMetricTimeSeries(evaluationId: string, metric: string, options?: TimeSeriesOptions): Promise<TimeSeriesData>
...
/**
 * Options for model metrics query
 */
interface ModelMetricOptions {
  evaluationId?: string
  startTime?: string
  endTime?: string
}
 ...
 */
interface ModelAggregatedMetrics {
  model: string
  totalRuns: number
  metrics: {
 ...
 */
async function loadEvalsDO(): Promise<new (ctx: MockDOState, env: MockEvalsEnv) => EvalsDOMetricsContract> {
...
describe('EvalsDO Metrics Collection', () => {
  ...
  let EvalsDO: new (ctx: MockDOState, env: MockEvalsEnv) => EvalsDOMetricsContract
  ...
  describe('collectMetric()', () => {
    ...
    it('should reject invalid metric values', async () => {
      const instance = new EvalsDO(ctx, env)
      await expect(
        instance.collectMetric('run-123', {
          model: 'gpt-4o',
          name: 'latencyMs',
          value: NaN,
        })
  ...
  describe('compareModels()', () => {
    it('should compare models by latency', async () => {
      ...
      const comparison = await instance.compareModels(
        ['gpt-4o', 'claude-sonnet-4-20250514', 'llama-3-70b'],
        'latencyMs'
      )
  ...
  describe('HTTP endpoints for metrics', () => {
    ...
    it('should handle GET /api/models/:model/metrics', async () => {
      ...
      expect(response.status).toBe(200)
      const data = await response.json() as ModelAggregatedMetrics
    ...
    it('should handle POST /api/models/compare', async () => {
      ...
      expect(response.status).toBe(200)
      const data = await response.json() as ModelComparison
    ...
    it('should handle GET /api/evaluations/:id/timeseries/:metric', async () => {
      ...
      expect(response.status).toBe(200)
      const data = await response.json() as TimeSeriesData
