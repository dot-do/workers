 ...
 */
export interface PrototypePollutionResult {
  isPolluted: boolean
...
export class PrototypePollutionError extends Error {
  public readonly result: PrototypePollutionResult

  constructor(message: string, result: PrototypePollutionResult) {
    super(message)
 ...
 */
export function hasPrototypePollutionKey(key: string): boolean {
 ...
 */
function hasCustomPrototype(obj: object): boolean {
 ...
 */
export function detectPrototypePollution(obj: unknown): PrototypePollutionResult {
  ...
  function traverse(current: unknown, path: string): void {
    ...
    for (const key of keys) {
      ...
      traverse((current as Record<string, unknown>)[key], newPath)
    }
  }
...
export function safeDeepMerge<T extends object>(
  ...
  for (const key of targetKeys) {
    if (!hasPrototypePollutionKey(key)) {
      result[key] = safeDeepClone((target as Record<string, unknown>)[key])
    }
  }
