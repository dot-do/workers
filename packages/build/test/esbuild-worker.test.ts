...
import { createESBuildWorker, type ESBuildWorker } from '../src/index.js'
  ...
  describe('Workers Environment Compatibility', () => {
    ...
    it('should handle large TypeScript files', async () => {
      ...
      const interfaces = Array.from(
        { length: 100 },
        (_, i) => `interface Type${i} { field${i}: number; }`
      ).join('\n')
    ...
    it('should handle concurrent compilations', async () => {
      ...
      const results = await Promise.all(
        sources.map(source => worker.compile(source))
      )
