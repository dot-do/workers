/**
 * Test helpers for humans.do worker tests
 *
 * Provides mock implementations for HumansDO testing.
 */
...
/**
 * Mock DOStorage interface for humans.do
 */
...
/**
 * Mock environment bindings for humans.do
 */
export interface MockHumansEnv {
  HUMANS_DO: {
 ...
 */
export function createMockEnv(): MockHumansEnv {
  return {
    HUMANS_DO: {
      get: vi.fn(),
 ...
 */
export function createMockTask(overrides?: Partial<HITLTask>): HITLTask {
  const now = new Date().toISOString()
  return {
    _id: `task-${Math.random().toString(36).slice(2, 10)}`,
    type: 'approval',
    title: 'Test Task',
    status: 'pending',
    priority: 'normal',
