...
import { createSourceMapManager } from '../src/source-maps.js';
describe('Source Map Manager - Production Source Maps', () => {
    ...
    describe('retrieve() - Source Map Retrieval', () => {
        ...
        it('should reject retrieval without authentication', async () => {
            await expect(manager.retrieve('deploy-001', { token: '' })).rejects.toThrow('Authentication required');
        });
        it('should reject retrieval with invalid token', async () => {
            await expect(manager.retrieve('deploy-001', { token: 'invalid-token' })).rejects.toThrow('Invalid authentication');
        });
    ...
    at a (bundle.js:1:15)`;
            await expect(manager.mapStackTrace('deploy-001', minifiedStack, { token: '' })).rejects.toThrow('Authentication required');
        });
    });
    ...
    describe('delete() - Source Map Deletion', () => {
        ...
        it('should require authentication for deletion', async () => {
            await expect(manager.delete('deploy-001', { token: '' })).rejects.toThrow('Authentication required');
        });
    ...
    describe('Security - Source Map Access Control', () => {
        ...
        it('should validate token scope for source map access', async () => {
            ...
            await expect(manager.retrieve('deploy-001', {
        ...
        it('should support IP allowlisting for access', async () => {
            ...
            await expect(restrictedManager.retrieve('deploy-001', {
                token: 'valid-token',
                clientIP: '8.8.8.8',
