...
import {
  ...
  safeJsonParse,
  freezePrototypes,
  type PrototypePollutionResult,
  PrototypePollutionError,
} from '../src/prototype-pollution.js'
...
describe('Prototype Pollution Prevention', () => {
  ...
  describe('safeDeepMerge', () => {
    ...
    it('handles nested merges safely', () => {
      ...
      const result = safeDeepMerge(target, source)
      expect((result as { user: { name: string; age: number } }).user.age).toBe(30)
    ...
    it('handles arrays correctly', () => {
      ...
      const result = safeDeepMerge(target, source)
      expect((result as { items: number[] }).items).toEqual([3, 4])
    })
  })
  ...
  describe('safeDeepClone', () => {
    ...
    it('strips constructor keys during clone', () => {
      ...
      const clone = safeDeepClone(original)
      expect((clone as Record<string, unknown>)['constructor']).toBeUndefined()
    })
  ...
  describe('safeJsonParse', () => {
    ...
    it('handles nested polluted objects', () => {
      ...
      const result = safeJsonParse(json)
      expect((result as { user: { name: string } }).user.name).toBe('John')
      expect(({} as Record<string, unknown>)['admin']).toBeUndefined()
    })
  ...
  describe('freezePrototypes', () => {
    ...
    it('prevents modifications to Object.prototype after freezing', () => {
      freezePrototypes()
      expect(() => {
        ;(Object.prototype as Record<string, unknown>)['newProp'] = 'value'
      }).toThrow()
    })
  ...
  describe('PrototypePollutionError', () => {
    it('should be throwable with detection result', () => {
      const result: PrototypePollutionResult = {
        isPolluted: true,
