...
/**
 * Interface definition for result serialization
 */
interface FunctionsDOSerializationContract {
  generate(prompt: string, options?: GenerateOptions): Promise<GenerateResult>
  extract<T>(text: string, schema: Record<string, unknown>): Promise<T>
 ...
 */
async function loadFunctionsDO(): Promise<new (ctx: MockDOState, env: MockFunctionsEnv) => FunctionsDOSerializationContract> {
...
describe('FunctionsDO Result Serialization', () => {
  ...
  let FunctionsDO: new (ctx: MockDOState, env: MockFunctionsEnv) => FunctionsDOSerializationContract
  ...
  describe('Special value serialization', () => {
    it('should handle null values in extracted data', async () => {
      ...
      env.AI.run = async <T>(): Promise<T> => {
        return { name: 'John', middleName: null, age: 30 } as T
      }

      const result = await instance.extract<{ name: string; middleName: string | null; age: number }>(
        'John, 30 years old',
        { type: 'object' }
      )
    ...
    it('should serialize Date objects as ISO strings', async () => {
      ...
      env.AI.run = async <T>(): Promise<T> => {
        return { event: 'meeting', date: new Date('2024-01-15T10:00:00Z') } as T
      }
    ...
    it('should handle special numeric values', async () => {
      ...
      env.AI.run = async <T>(): Promise<T> => {
        return { data: [[0.000001, 0.999999, -0.5, 1e-10]] } as T
      }
    ...
    it('should handle BigInt by converting to number or string', async () => {
      ...
      env.AI.run = async <T>(): Promise<T> => {
        return { count: BigInt(123456789) } as T
      }
  ...
  describe('Function definition serialization', () => {
    ...
    it('should serialize JSON schema parameters correctly', async () => {
      ...
      expect((complexFunc?.parameters as any)?.type).toBe('object')
    })
  })
})
