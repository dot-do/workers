...
import {
  ...
  type MockCloudflareAPI,
} from './helpers.js'

/**
 * Interface definition for DeployerDO Cloudflare API integration
 */
interface DeployerDOCloudflareContract {
  // Configuration
  setCloudflareCredentials(credentials: CloudflareCredentials): Promise<void>
  getCloudflareStatus(): Promise<CloudflareStatus>

  // Account operations
  listAccounts(): Promise<CloudflareAccount[]>
  setActiveAccount(accountId: string): Promise<void>
  getActiveAccount(): Promise<CloudflareAccount | null>

  // Namespace operations
  listNamespaces(): Promise<WorkerNamespace[]>
  createNamespace(name: string): Promise<WorkerNamespace>
  ...
  // Direct API access
  callCloudflareApi<T>(endpoint: string, options?: ApiCallOptions): Promise<T>
 ...
 */
async function loadDeployerDO(): Promise<new (ctx: MockDOState, env: MockDeployerEnv) => DeployerDOCloudflareContract> {
...
describe('DeployerDO Cloudflare API Integration', () => {
  ...
  let DeployerDO: new (ctx: MockDOState, env: MockDeployerEnv) => DeployerDOCloudflareContract
  let mockCloudflareApi: MockCloudflareAPI
  ...
  describe('API error handling', () => {
    it('should handle authentication errors', async () => {
      ...
      // Mock API to reject
      vi.mocked(mockCloudflareApi.workers.scripts.list).mockRejectedValueOnce(
        new Error('Authentication error: Invalid API token')
      )
    ...
    it('should retry on transient failures', async () => {
      ...
      const result = await instance.callCloudflareApi<unknown[]>('/workers/scripts')
    ...
    it('should not expose API token in error messages', async () => {
      ...
      vi.mocked(mockCloudflareApi.workers.scripts.list).mockRejectedValueOnce(
        new Error('API error with token: super-secret-token')
      )
  ...
  describe('callCloudflareApi() - Direct API access', () => {
    it('should make GET requests', async () => {
      ...
      const result = await instance.callCloudflareApi<{ success: boolean }>('/zones')
    ...
    it('should make POST requests with body', async () => {
      ...
      const result = await instance.callCloudflareApi<{ id: string }>('/workers/scripts', {
        method: 'POST',
  ...
  describe('Script upload with Cloudflare API', () => {
    ...
    it('should handle upload errors gracefully', async () => {
      ...
      vi.mocked(mockCloudflareApi.workers.scripts.create).mockRejectedValueOnce(
        new Error('Script size exceeds limit')
      )
  ...
  describe('Deployment with Cloudflare API', () => {
    ...
    it('should pass deployment options to API', async () => {
      ...
      expect(mockCloudflareApi.workers.deployments.create).toHaveBeenCalledWith(
  ...
  describe('HTTP endpoints for Cloudflare integration', () => {
    ...
    it('should handle GET /api/cloudflare/accounts', async () => {
      ...
      expect(response.status).toBe(200)
      const accounts = await response.json() as CloudflareAccount[]
    ...
    it('should handle GET /api/cloudflare/namespaces', async () => {
      ...
      expect(response.status).toBe(200)
      const namespaces = await response.json() as WorkerNamespace[]
      expect(Array.isArray(namespaces)).toBe(true)
    })
    ...
    it('should handle POST /api/cloudflare/namespaces', async () => {
      ...
      const namespace = await response.json() as WorkerNamespace
