...
import { DOCore, DOState, DOStorage, DurableObjectId, SqlStorage, SqlStorageCursor } from '../src/index.js'
...
function createMockSqlStorage(): SqlStorage {
  ...
  return {
    exec: vi.fn(<T>(query: string, ..._bindings: unknown[]): SqlStorageCursor<T> => {
...
function createMockStorage(): DOStorage {
  ...
  const storage: DOStorage = {
    ...
    list: vi.fn(async <T>(options?: { prefix?: string; start?: string; end?: string; limit?: number }) => {
      const result = new Map<string, T>()
...
describe('State Persistence Contract', () => {
  ...
  describe('Single Key Operations', () => {
    ...
    it('should store arrays', async () => {
      ...
      const value = await storage.get<typeof arr>('array')
  ...
  describe('Batch Operations', () => {
    it('should get multiple keys at once', async () => {
      ...
      const result = await storage.get<number>(['a', 'b', 'c'])

      expect(result).toBeInstanceOf(Map)
      expect((result as Map<string, number>).get('a')).toBe(1)
      expect((result as Map<string, number>).get('b')).toBe(2)
      expect((result as Map<string, number>).get('c')).toBe(3)
    })

    it('should only return existing keys in batch get', async () => {
      await storage.put('a', 1)

      const result = await storage.get<number>(['a', 'nonexistent'])

      expect(result).toBeInstanceOf(Map)
      expect((result as Map<string, number>).size).toBe(1)
      expect((result as Map<string, number>).has('nonexistent')).toBe(false)
    })
  ...
  describe('Transaction Support', () => {
    ...
    it('should support read-modify-write pattern', async () => {
      ...
      expect(await storage.get<number>('counter')).toBe(1)
    })
  ...
  describe('SQL Storage Interface', () => {
    ...
    it('should execute SELECT statements and return cursor', () => {
      ...
      const cursor = storage.sql.exec<{ id: string; value: string }>(
  ...
  describe('DOCore with Storage', () => {
    it('should allow subclass to use storage', async () => {
      ...
      const getResponse = await instance.fetch(
        new Request('https://example.com/?key=test')
      )
      const data = await getResponse.json() as { value: string }
