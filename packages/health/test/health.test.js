...
import { HealthChecker, HealthStatus, } from '../src/index';
describe('HealthChecker', () => {
    ...
    describe('Dependency Health Checks', () => {
        ...
        it('should handle dependency check timeout', async () => {
            const slowCheck = vi.fn().mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 10000)));
    ...
    describe('Aggregated Health Status', () => {
        ...
        it('should run all dependency checks in parallel', async () => {
            const startTimes = [];
            const endTimes = [];
            const createSlowCheck = (delay) => async () => {
                startTimes.push(Date.now());
                await new Promise((resolve) => setTimeout(resolve, delay));
                endTimes.push(Date.now());
                return { name: `check-${delay}`, status: 'healthy' };
            };
    ...
    describe('Configuration', () => {
        ...
        it('should allow overriding timeout per dependency', async () => {
            const slowCheck = vi.fn().mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 200)));
