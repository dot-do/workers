...
import { ActionsMixin, ActionsBase } from '../src/actions-mixin.js';
// Mock implementations
function createMockId(name) {
    return {
        name,
        toString: () => name ?? 'mock-id',
        equals: (other) => other.toString() === (name ?? 'mock-id'),
    };
}
...
function createMockStorage() {
    const store = new Map();
    let alarmTime = null;
    const getMock = vi.fn(async (keyOrKeys) => {
...
describe('ActionsMixin', () => {
    ...
    describe('Action Execution', () => {
        it('should execute a simple action', async () => {
            ...
            const result = await actions.executeAction('ping');
        ...
        it('should pass parameters to action handler', async () => {
            ...
            const result = await actions.executeAction('greet', { name: 'World' });
            expect(result.success).toBe(true);
    ...
    describe('Middleware Support', () => {
        ...
        it('should allow middleware to modify result', async () => {
            ...
            const result = await actions.executeAction('modify');
            expect(result.success).toBe(true);
    ...
    describe('ActionsBase Convenience Class', () => {
        ...
        it('should work like composed mixin', async () => {
            ...
            const result = await base.executeAction('test');
            expect(result.success).toBe(true);
