...
import { createTestContext, withTestContext, createFixture, useFixture, registerCleanup, runCleanup, assertRequest, assertResponse, assertDurableObjectState, } from '../src/context/index.js';
import { createMockRequest, createMockResponse, createMockDurableObjectState, createMockExecutionContext, createMockKVNamespace, } from '../src/mocks/index.js';
/**
...
describe('Test Context Utilities', () => {
    describe('createTestContext', () => {
        ...
        it('should flush all waitUntil promises', async () => {
            const ctx = createTestContext();
            let resolved = false;
            ctx.executionContext.waitUntil(new Promise((resolve) => {
                setTimeout(() => {
                    resolved = true;
                    resolve();
                }, 10);
    ...
    describe('withTestContext', () => {
        ...
        it('should cleanup even if test throws', async () => {
            let cleanedUp = false;
            await expect(withTestContext(async (ctx) => {
                registerCleanup(ctx, () => {
                    cleanedUp = true;
                });
    ...
    describe('createFixture', () => {
        ...
        it('should support fixture factory with options', () => {
            const userFixture = createFixture('user', (options) => ({
                id: 'user-123',
                name: 'Test User',
    ...
    describe('useFixture', () => {
        it('should use a fixture within a test context', async () => {
            const requestFixture = createFixture('request', () => createMockRequest({ url: 'https://example.com/api' }));
            await withTestContext(async (ctx) => {
        ...
        it('should automatically cleanup fixture on context teardown', async () => {
            let disposed = false;
            const disposableFixture = createFixture('disposable', () => ({ value: 'test' }), () => { disposed = true; });
            await withTestContext(async (ctx) => {
    ...
    describe('Assertion Helpers', () => {
        describe('assertRequest', () => {
            ...
            it('should assert request body as JSON', async () => {
                ...
                await expect(assertRequest(request, { jsonBody: { name: 'test' } })).resolves.not.toThrow();
            });
        });
        describe('assertResponse', () => {
            ...
            it('should assert response headers', () => {
                ...
                expect(() => assertResponse(response, {
                    headers: { 'X-Custom': 'value' },
                })).not.toThrow();
            });
            it('should assert response body as JSON', async () => {
                ...
                await expect(assertResponse(response, { jsonBody: { success: true } })).resolves.not.toThrow();
            });
        });
        describe('assertDurableObjectState', () => {
            it('should assert storage contains key', async () => {
                ...
                await expect(assertDurableObjectState(state, { hasKey: 'myKey' })).resolves.not.toThrow();
                await expect(assertDurableObjectState(state, { hasKey: 'nonexistent' })).rejects.toThrow();
            });
            it('should assert storage value', async () => {
                const state = createMockDurableObjectState();
                await state.storage.put('count', 42);
                await expect(assertDurableObjectState(state, { keyValue: { count: 42 } })).resolves.not.toThrow();
                await expect(assertDurableObjectState(state, { keyValue: { count: 0 } })).rejects.toThrow();
            });
            it('should assert storage key count', async () => {
                const state = createMockDurableObjectState();
                await state.storage.put('a', 1);
                await state.storage.put('b', 2);
                await expect(assertDurableObjectState(state, { keyCount: 2 })).resolves.not.toThrow();
                await expect(assertDurableObjectState(state, { keyCount: 3 })).rejects.toThrow();
            });
        });
    });
});
