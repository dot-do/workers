...
export interface ExecutorConfig {
  /** Maximum timeout allowed (default: 30000ms) */
  maxTimeout?: number
  /** Enable strict mode for executed code */
  strictMode?: boolean
...
/**
 * CodeExecutor interface - Workers-compatible sandboxed execution
 */
export interface CodeExecutor {
  ...
  /**
   * Check if code is syntactically valid without executing
   */
 ...
 */
function createFrozenBuiltins(): Record<string, unknown> {
 ...
 */
function createSandboxedObject(): typeof Object {
  const SandboxedObject = function SandboxedObject(this: unknown, value?: unknown) {
  ...
  for (const method of staticMethods) {
    if (method in Object) {
      ...
      if (method === 'setPrototypeOf' as string) {
        (SandboxedObject as unknown as Record<string, unknown>)[method] = function(obj: object, proto: object | null) {
        ...
        (SandboxedObject as unknown as Record<string, unknown>)[method] = (Object as unknown as Record<string, unknown>)[method]
      }
    }
  }

  // Add setPrototypeOf with protection
  (SandboxedObject as unknown as Record<string, unknown>).setPrototypeOf = function(obj: object, proto: object | null) {
    // Block modification of built-in prototypes
    if (obj === Array.prototype || obj === Object.prototype ||
        obj === Function.prototype || obj === String.prototype ||
        obj === Number.prototype) {
 ...
 */
function createSandboxedArray(): typeof Array {
  const SandboxedArray = function SandboxedArray(this: unknown, ...args: unknown[]) {
    return new Array(...args)
  } as unknown as typeof Array

  // Copy static methods
  const staticMethods = ['isArray', 'from', 'of'] as const
  for (const method of staticMethods) {
    (SandboxedArray as unknown as Record<string, unknown>)[method] = Array[method]
  }
 ...
 */
function createSandboxedFunction(): typeof Function {
 ...
 */
function createSandboxedReflect(): typeof Reflect {
  ...
  for (const method of safeMethods) {
    if (method in Reflect) {
      (SandboxedReflect as Record<string, unknown>)[method] = (Reflect as Record<string, unknown>)[method]
    }
  }

  // Wrap setPrototypeOf to prevent modifying built-in prototypes
  SandboxedReflect.setPrototypeOf = function(target: object, proto: object | null) {
    // Block modification of built-in prototypes
  ...
  SandboxedReflect.getPrototypeOf = function(target: object) {
    return Reflect.getPrototypeOf(target)
  }

  Object.freeze(SandboxedReflect)
  return SandboxedReflect as typeof Reflect
}
 ...
 */
function parseErrorLocation(error: Error): { line: number; column: number } | undefined {
 ...
 */
function sanitizeErrorStack(error: string): string {
 ...
 */
function instrumentWithTimeoutChecks(code: string, timeoutVar: string): string {
  ...
  let result = code.replace(
    /\b(while\s*\([^)]*\)\s*\{)/g,
    `$1 if(Date.now() > ${timeoutVar}) throw new Error('Execution timeout');`
  )
 ...
 */
function containsDangerousPatterns(code: string): { dangerous: boolean; pattern?: string } {
 ...
 */
function generateSecurityPreamble(): string {
 ...
 */
class CodeExecutorImpl implements CodeExecutor {
 ...
 */
export class Evaluator implements CodeExecutor {
 ...
 */
export function createEvaluator(config?: ExecutorConfig): Evaluator {
...
export interface TranspileResult {
  ...
  error?: string
  /** Location of error if transpilation failed */
  location?: { line: number; column: number }
...
export interface TranspileOptions {
  ...
  /** Module system (default: ESNext) */
  module?: 'CommonJS' | 'ES2015' | 'ES2020' | 'ESNext' | 'None'
  /** Enable strict mode */
  strict?: boolean
  /** JSX handling */
...
/**
 * TypeScriptTranspiler interface - Workers-compatible TypeScript compilation
 */
export interface TypeScriptTranspiler {
  ...
  /**
   * Dispose of the transpiler and clean up resources
   */
  dispose(): void
}
 ...
 */
function mapTarget(target: TranspileOptions['target']): ts.ScriptTarget {
  switch (target) {
 ...
 */
function mapModule(module: TranspileOptions['module']): ts.ModuleKind {
  switch (module) {
 ...
 */
function mapJsx(jsx: TranspileOptions['jsx']): ts.JsxEmit | undefined {
  switch (jsx) {
 ...
 */
class TypeScriptTranspilerImpl implements TypeScriptTranspiler {
  private disposed = false
  private config: TranspileOptions

  constructor(config: TranspileOptions = {}) {
    this.config = {
   ...
   */
  private checkInvalidDecorators(code: string): TranspileResult | undefined {
    ...
    let invalidDecoratorError: TranspileResult | undefined

    const visit = (node: ts.Node) => {
      if (invalidDecoratorError) return // Already found an error
      ...
      let firstDecoratorChild: ts.Decorator | undefined
      ts.forEachChild(node, (child) => {
        if (child.kind === ts.SyntaxKind.Decorator && !firstDecoratorChild) {
          firstDecoratorChild = child as ts.Decorator
        }
      })
      ...
      if (firstDecoratorChild) {
        ...
        if (!isValidTarget) {
          const { line, character } = sourceFile.getLineAndCharacterOfPosition(
            firstDecoratorChild.getStart(sourceFile)
          )
 ...
 */
export function createTypeScriptTranspiler(config?: TranspileOptions): TypeScriptTranspiler {
