...
import { SessionManager, SessionStore, generateSecureToken, hashToken, createSession, validateSession, destroySession, refreshSession, } from '../src/index';
/**
...
describe('SessionStore Interface', () => {
    describe('KV-based SessionStore', () => {
        ...
        it('should save a session to KV', async () => {
            ...
            expect(mockKV.put).toHaveBeenCalledWith(expect.stringContaining(session.id), expect.any(String), expect.objectContaining({ expirationTtl: expect.any(Number) }));
        });
...
describe('SessionManager', () => {
    ...
    describe('User Session Management', () => {
        it('should list all sessions for a user', async () => {
            ...
            mockKV.get.mockImplementation((key) => {
                if (key === 'session:user-123:session-1')
                    return JSON.stringify(mockSession1);
                if (key === 'session:user-123:session-2')
    ...
    describe('Session Configuration', () => {
        ...
        it('should support session prefix configuration', async () => {
            ...
            const session = await customManager.create({ userId: 'user-123' });
            expect(mockKV.put).toHaveBeenCalledWith(expect.stringContaining('custom-session:'), expect.any(String), expect.any(Object));
        });
    });
