...
/**
 * Interface definition for EvalsDO - this defines the contract
 * The implementation must satisfy this interface
 */
export interface EvalsDOContract {
  ...
  listEvaluations(options?: ListEvaluationsOptions): Promise<Evaluation[]>
  deleteEvaluation(id: string): Promise<boolean>

  // Run evaluations
  runEvaluation(evaluationId: string, options?: RunOptions): Promise<EvaluationRun>
  getEvaluationRun(runId: string): Promise<EvaluationRun | null>
  listEvaluationRuns(evaluationId: string, options?: ListRunsOptions): Promise<EvaluationRun[]>
...
/**
 * Configuration for creating an evaluation
 */
...
/**
 * Prompt template for parameterized evaluations
 */
...
/**
 * Results for a single model in an evaluation run
 */
 ...
 */
async function loadEvalsDO(): Promise<new (ctx: MockDOState, env: MockEvalsEnv) => EvalsDOContract> {
...
describe('EvalsDO RPC Interface', () => {
  ...
  let EvalsDO: new (ctx: MockDOState, env: MockEvalsEnv) => EvalsDOContract
  ...
  describe('HTTP fetch() handler', () => {
    describe('RPC endpoint', () => {
      it('should handle POST /rpc with method call', async () => {
        ...
        const result = await response.json() as { result: Evaluation }
    ...
    describe('REST API endpoints', () => {
      ...
      it('should handle GET /api/evaluations/:id/runs', async () => {
        ...
        expect(response.status).toBe(200)
        const runs = await response.json() as EvaluationRun[]
