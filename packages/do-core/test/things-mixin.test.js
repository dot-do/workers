...
import { applyThingsMixin } from '../src/things-mixin.js';
...
function createMockStateWithSql() {
    ...
    const sqlStorage = {
        exec: vi.fn((query, ...params) => {
            ...
            if (normalizedQuery.startsWith('delete') && normalizedQuery.includes('things')) {
                ...
                const remaining = things.filter((t) => {
                    const thing = t;
                    return !(thing.params[0] === ns && thing.params[1] === type && thing.params[2] === id);
                });
            ...
            if (normalizedQuery.startsWith('select') && normalizedQuery.includes('from things')) {
                ...
                return createMockSqlCursor(results);
            }
            return createMockSqlCursor([]);
        }),
    };
...
describe('ThingsMixin', () => {
    ...
    describe('Event Handling', () => {
        ...
        it('should not throw if event handler errors', async () => {
            ...
            // Should not throw
            await expect(instance.createThing({ type: 'user', data: { name: 'Test' } })).resolves.toBeDefined();
        });
    });
    describe('Schema Initialization', () => {
        ...
        it('should only initialize schema once', async () => {
            ...
            const calls = state.storage.sql.exec.mock.calls;
            const createCalls = calls.filter((c) => c[0].toLowerCase().includes('create table'));
    ...
    describe('Type Safety', () => {
        it('should properly type Thing data', async () => {
            ...
            const _name = thing.data.name;
            const _age = thing.data.age;
