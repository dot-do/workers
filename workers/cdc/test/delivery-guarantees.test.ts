...
/**
 * Interface for delivery guarantee testing
 */
interface CDCDeliveryContract {
  ...
  negativeAck(pipelineId: string, eventId: string, consumerId: string, reason?: string): Promise<DeliveryAck>

  // Consumer management
  registerConsumer(pipelineId: string, consumerId: string): Promise<ConsumerState>
  unregisterConsumer(pipelineId: string, consumerId: string): Promise<boolean>
  getConsumerState(pipelineId: string, consumerId: string): Promise<ConsumerState | null>
  listConsumers(pipelineId: string): Promise<ConsumerState[]>
  ...
  commitCheckpoint(pipelineId: string, consumerId: string): Promise<void>

  // Delivery tracking
  getPendingDeliveries(pipelineId: string, consumerId: string, limit?: number): Promise<CDCEvent[]>
  getDeliveryStatus(pipelineId: string, eventId: string): Promise<Record<string, DeliveryAck>>
  retryDelivery(pipelineId: string, eventId: string, consumerId: string): Promise<DeliveryAck>

  // Replay
  replayEvents(pipelineId: string, fromSequence: number, toSequence?: number): Promise<CDCEvent[]>
  replayForConsumer(pipelineId: string, consumerId: string, fromCheckpoint?: number): Promise<CDCEvent[]>
}

/**
 * Attempt to load CDCDO - this will fail in RED phase
 */
async function loadCDCDO(): Promise<new (ctx: MockDOState, env: MockCDCEnv) => CDCDeliveryContract> {
...
describe('CDC Delivery Guarantees', () => {
  let ctx: MockDOState
  let env: MockCDCEnv
  let CDCDO: new (ctx: MockDOState, env: MockCDCEnv) => CDCDeliveryContract
  ...
  describe('At-Least-Once Delivery', () => {
    ...
    it('should move to dead letter after max retries', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (i < 2) await instance.retryDelivery('max-retries', event.id, 'consumer-1')
      }
