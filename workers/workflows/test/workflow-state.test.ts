...
/**
 * Workflow state stored in durable storage
 */
...
/**
 * Execution state stored in durable storage
 */
export interface StoredExecutionState {
  ...
  /** Error message */
  error?: string
  /** Resume point for paused workflows */
  resumePoint?: {
    stepId: string
    stepIndex: number
    retryCount: number
...
/**
 * Interface for state management methods
 */
interface WorkflowsStateContract {
  // State getters
  getWorkflowState(workflowId: string): Promise<StoredWorkflowState | null>
  getExecutionState(executionId: string): Promise<StoredExecutionState | null>

  // State persistence
  saveWorkflowState(state: StoredWorkflowState): Promise<void>
  saveExecutionState(state: StoredExecutionState): Promise<void>

  // State querying
  findExecutionsByStatus(status: string): Promise<StoredExecutionState[]>
  findExecutionsByWorkflow(workflowId: string): Promise<StoredExecutionState[]>

  // Cleanup
  deleteWorkflowState(workflowId: string): Promise<boolean>
  deleteExecutionState(executionId: string): Promise<boolean>
  pruneOldExecutions(olderThanMs: number): Promise<number>
}

/**
 * Attempt to load WorkflowsDO - this will fail in RED phase
 */
async function loadWorkflowsDO(): Promise<new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsStateContract> {
...
describe('WorkflowsDO State Management', () => {
  ...
  let WorkflowsDO: new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsStateContract
