...
/**
 * Interface for timeout management operations
 */
interface HumansDOTimeoutContract {
  // Task creation with timeout
  ...
  // Timeout operations
  setTaskTimeout(taskId: string, timeoutMs: number): Promise<HITLTask | null>
  clearTaskTimeout(taskId: string): Promise<HITLTask | null>
  extendTimeout(taskId: string, additionalMs: number): Promise<HITLTask | null>

  // Expiration queries
  getExpiredTasks(): Promise<HITLTask[]>
  getExpiringTasks(withinMs: number): Promise<HITLTask[]>

  // Alarm handler (called by Durable Object runtime)
  alarm(): Promise<void>
 ...
 */
async function loadHumansDO(): Promise<new (ctx: MockDOState, env: MockHumansEnv) => HumansDOTimeoutContract> {
...
describe('HumansDO Timeout Management', () => {
  ...
  let HumansDO: new (ctx: MockDOState, env: MockHumansEnv) => HumansDOTimeoutContract
  ...
  describe('extendTimeout()', () => {
    ...
    it('should throw error for task without timeout', async () => {
      ...
      await expect(
        instance.extendTimeout(created._id, 1800000)
      ).rejects.toThrow(/no timeout|not set/i)
    })
  })
  ...
  describe('HTTP endpoints for timeout', () => {
    ...
    it('should handle GET /api/tasks/expired', async () => {
      ...
      const response = await instance.fetch(request)
      expect(response.status).toBe(200)
      const tasks = await response.json() as HITLTask[]
