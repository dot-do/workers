...
import { CircuitBreaker, CircuitBreakerState, CircuitBreakerError } from '../src/index';
describe('CircuitBreaker', () => {
    ...
    describe('timeout handling', () => {
        ...
        it('should timeout slow operations', async () => {
            const slowOperation = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('slow result'), 2000)));
        ...
        it('should count timeout as a failure', async () => {
            const slowOperation = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('slow'), 2000)));
            const executePromise = breaker.execute(slowOperation);
            vi.advanceTimersByTime(1001);
            try {
        ...
        it('should complete successfully if operation finishes before timeout', async () => {
            const fastOperation = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('fast result'), 500)));
    ...
    describe('custom failure predicate', () => {
        it('should use custom predicate to determine failures', async () => {
            ...
            const isFailure = (error) => error.message.startsWith('5');
            breaker = new CircuitBreaker({
                failureThreshold: 2,
                successThreshold: 2,
                timeout: 1000,
