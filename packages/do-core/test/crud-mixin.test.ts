...
class TestDOBase extends CRUDBase {
  ...
  constructor(ctx: DOState, _env: DOEnv) {
    super()
    this.ctx = ctx
  }
...
describe('CRUDMixin', () => {
  ...
  describe('get()', () => {
    it('should return null for non-existent document', async () => {
      const result = await instance.get<User>('users', 'nonexistent')
    ...
    it('should return document if it exists', async () => {
      ...
      const result = await instance.get<User>('users', '123')
  ...
  describe('create()', () => {
    it('should create document with auto-generated id', async () => {
      ...
      const created = await instance.create<typeof data>('users', data)
    ...
    it('should store document in storage with collection prefix', async () => {
      ...
      await instance.create<typeof data>('users', data)

      const stored = await ctx.storage.get<typeof data>('users:test-id')
  ...
  describe('update()', () => {
    it('should return null if document does not exist', async () => {
      const result = await instance.update<User>('users', 'nonexistent', { name: 'Updated' })
      expect(result).toBeNull()
    })

    it('should update existing document', async () => {
      const user: User = { id: '123', name: 'Frank', email: 'frank@example.com', createdAt: 1000 }
      await ctx.storage.put('users:123', user)
    ...
    it('should update the updatedAt timestamp', async () => {
      const user: User = { id: '123', name: 'Grace', email: 'grace@example.com', updatedAt: 1000 }
      await ctx.storage.put('users:123', user)

      const before = Date.now()
      const updated = await instance.update<User>('users', '123', { name: 'Gracie' })
    ...
    it('should not allow changing the id', async () => {
      const user: User = { id: '123', name: 'Henry', email: 'henry@example.com' }
      await ctx.storage.put('users:123', user)

      const updated = await instance.update<User>('users', '123', { id: 'new-id', name: 'Harry' } as Partial<User>)
  ...
  describe('delete()', () => {
    ...
    it('should delete existing document', async () => {
      const user: User = { id: '123', name: 'Ivy', email: 'ivy@example.com' }
      await ctx.storage.put('users:123', user)
  ...
  describe('list()', () => {
    ...
    it('should list documents in a collection', async () => {
      const users = await instance.list<User>('users')
    ...
    it('should not include documents from other collections', async () => {
      ...
      expect(users.some(u => (u as unknown as { title: string }).title === 'Post 1')).toBe(false)
    })

    it('should respect limit option', async () => {
      const users = await instance.list<User>('users', { limit: 2 })

      expect(users).toHaveLength(2)
    })

    it('should respect offset option', async () => {
      const allUsers = await instance.list<User>('users')
      const offsetUsers = await instance.list<User>('users', { offset: 1 })
    ...
    it('should return empty array for non-existent collection', async () => {
      const empty = await instance.list<User>('nonexistent')
  ...
  describe('upsert()', () => {
    it('should create document if it does not exist', async () => {
      ...
      const result = await instance.upsert<typeof data>('users', 'jack', data)
    ...
    it('should update document if it exists', async () => {
      const existing: User = { id: 'kate', name: 'Kate', email: 'kate@example.com', createdAt: 1000 }
      await ctx.storage.put('users:kate', existing)

      const result = await instance.upsert<Partial<User>>('users', 'kate', { name: 'Katherine' })
  ...
  describe('getMany()', () => {
    ...
    it('should return empty map for no ids', async () => {
      const result = await instance.getMany<User>('users', [])
      expect(result.size).toBe(0)
    })

    it('should return map of found documents', async () => {
      const result = await instance.getMany<User>('users', ['alice', 'bob'])
    ...
    it('should not include non-existent ids', async () => {
      const result = await instance.getMany<User>('users', ['alice', 'nonexistent'])
...
describe('CRUDBase', () => {
  ...
  describe('basic operations', () => {
    ...
    it('should support update()', async () => {
      ...
      const updated = await instance.update<User>('users', '123', { name: 'Updated' })
