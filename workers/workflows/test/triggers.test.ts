...
export interface EventTrigger {
  type: 'event'
  /** Event pattern (e.g., "$.on.user.created", "$.on.order.completed") */
  pattern: string
...
export interface ScheduleTrigger {
  type: 'schedule'
  /** Schedule pattern (e.g., "$.every.5.minutes", "$.every.1.hour") */
  pattern: string
...
export type WorkflowTrigger = EventTrigger | ScheduleTrigger | WebhookTrigger | ManualTrigger
...
/**
 * Interface for trigger management methods
 */
interface WorkflowsTriggersContract {
  // Trigger registration
  registerTrigger(workflowId: string, trigger: WorkflowTrigger): Promise<RegisteredTrigger>
  unregisterTrigger(workflowId: string): Promise<boolean>
  getTrigger(workflowId: string): Promise<RegisteredTrigger | null>
  listTriggers(options?: { type?: string; enabled?: boolean }): Promise<RegisteredTrigger[]>

  // Trigger control
  enableTrigger(workflowId: string): Promise<boolean>
  disableTrigger(workflowId: string): Promise<boolean>

  // Event handling
  handleEvent(event: EventPayload): Promise<Array<{ workflowId: string; executionId: string }>>
  matchEventPattern(pattern: string, eventType: string): boolean

  // Schedule handling
  getNextScheduledRun(workflowId: string): Promise<number | null>
  checkScheduledWorkflows(): Promise<Array<{ workflowId: string; executionId: string }>>
 ...
 */
async function loadWorkflowsDO(): Promise<
  new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsTriggersContract
> {
...
describe('WorkflowsDO Trigger Management', () => {
  ...
  let WorkflowsDO: new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsTriggersContract
  ...
  describe('Trigger Registration', () => {
    describe('registerTrigger()', () => {
      ...
      it('should overwrite existing trigger for same workflow', async () => {
        ...
        expect((updated.trigger as EventTrigger).pattern).toBe('$.on.new')
      })
      ...
      it('should validate event pattern format', async () => {
        ...
        await expect(instance.registerTrigger('workflow-1', invalidTrigger)).rejects.toThrow(
          /invalid.*pattern/i
        )
      })
      ...
      it('should validate schedule pattern format', async () => {
        const instance = new WorkflowsDO(ctx, env)
        const invalidTrigger: ScheduleTrigger = {
          type: 'schedule',
          pattern: 'invalid-schedule', // Should be $.every.N.unit
        }

        await expect(instance.registerTrigger('workflow-1', invalidTrigger)).rejects.toThrow(
          /invalid.*pattern|schedule/i
        )
      })
    })
    ...
    describe('getTrigger()', () => {
      ...
      it('should return registered trigger info', async () => {
        ...
        expect((trigger!.trigger as EventTrigger).filter).toEqual({ role: 'admin' })
      })
    })
  ...
  describe('Webhook Handling', () => {
    describe('handleWebhook()', () => {
      ...
      it('should trigger workflow for matching webhook', async () => {
        ...
        const result = await instance.handleWebhook(
          '/hooks/process-order',
          'POST',
          { orderId: '123' },
          {}
