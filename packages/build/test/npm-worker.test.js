...
import { createNpmWorker } from '../src/index.js';
describe('NPM Worker - Package Publishing', () => {
    ...
    describe('publish() - Basic Package Publishing', () => {
        ...
        it('should support ArrayBuffer tarball', async () => {
            ...
            const tarballBuffer = encoder.encode('fake-tarball-content').buffer;
            const pkg = {
    ...
    describe('versionExists() - Version Checking', () => {
        ...
        it('should support custom registry', async () => {
            const exists = await worker.versionExists('@company/private-package', '1.0.0', {
                token: 'npm_test_token',
                registry: 'https://npm.pkg.github.com',
            });
        ...
        it('should handle authentication for private packages', async () => {
            const exists = await worker.versionExists('@dotdo/private-package', '1.0.0', { token: 'npm_test_token' });
            expect(typeof exists).toBe('boolean');
        });
    });
    describe('getPackageInfo() - Package Metadata', () => {
        ...
        it('should support authentication for private packages', async () => {
            const info = await worker.getPackageInfo('@dotdo/private-package', { token: 'npm_test_token' });
            // Should not throw, may return null if package doesn't exist
            expect(info === null || typeof info === 'object').toBe(true);
        });
        it('should support custom registry', async () => {
            const info = await worker.getPackageInfo('@company/private-package', {
                token: 'npm_test_token',
                registry: 'https://npm.pkg.github.com',
            });
            expect(info === null || typeof info === 'object').toBe(true);
    ...
    describe('unpublish() - Package Removal', () => {
        ...
        it('should unpublish a specific version', async () => {
            const result = await worker.unpublish('@dotdo/test-package', '1.0.0', { token: 'npm_test_token' });
            expect(result.success).toBe(true);
        });
        it('should fail without authentication', async () => {
            const result = await worker.unpublish('@dotdo/test-package', '1.0.0', { token: '' });
            expect(result.success).toBe(false);
            expect(result.error).toBeDefined();
        });
        it('should fail for non-existing package', async () => {
            const result = await worker.unpublish('@dotdo/non-existing-package', '1.0.0', { token: 'npm_test_token' });
            expect(result.success).toBe(false);
            expect(result.error).toBeDefined();
        });
        it('should fail for package owned by someone else', async () => {
            const result = await worker.unpublish('lodash', '4.17.21', { token: 'npm_test_token' });
            expect(result.success).toBe(false);
            expect(result.error).toBeDefined();
        });
        it('should support custom registry', async () => {
            const result = await worker.unpublish('@company/private-package', '1.0.0', {
                token: 'npm_test_token',
                registry: 'https://npm.pkg.github.com',
            });
