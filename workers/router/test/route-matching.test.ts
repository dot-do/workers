...
/**
 * Interface definition for RouterDO route matching
 */
export interface RouterDOContract {
  // Route management
  addRoute(hostname: string, route: RouteConfig): Promise<void>
  removeRoute(hostname: string, pattern: string): Promise<boolean>
  listRoutes(hostname: string): Promise<RouteConfig[]>

  // Route matching
  matchRoute(hostname: string, path: string, method?: string): Promise<RouteConfig | null>

  // Hostname config (from hostname-routing tests)
