...
/**
 * Interface definition for HumansDO - this defines the contract
 * The implementation must satisfy this interface
 */
export interface HumansDOContract {
  // Task creation
 ...
 */
async function loadHumansDO(): Promise<new (ctx: MockDOState, env: MockHumansEnv) => HumansDOContract> {
...
describe('HumansDO RPC Interface', () => {
  ...
  let HumansDO: new (ctx: MockDOState, env: MockHumansEnv) => HumansDOContract
  ...
  describe('HTTP fetch() handler', () => {
    describe('RPC endpoint', () => {
      it('should handle POST /rpc with createTask', async () => {
        ...
        const result = await response.json() as { result: HITLTask }
