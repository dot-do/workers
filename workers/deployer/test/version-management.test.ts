...
/**
 * Interface definition for DeployerDO version management
 */
interface DeployerDOVersionContract {
  // Script operations (from deployer-rpc.test.ts)
  uploadScript(params: UploadScriptParams): Promise<ScriptInfo>
  getScript(scriptName: string): Promise<ScriptInfo | null>

  // Version operations
  createVersion(params: CreateVersionParams): Promise<VersionInfo>
  getVersion(scriptName: string, versionId: string): Promise<VersionInfo | null>
  listVersions(scriptName: string, options?: ListVersionsOptions): Promise<VersionInfo[]>
  getLatestVersion(scriptName: string): Promise<VersionInfo | null>
  getActiveVersion(scriptName: string): Promise<VersionInfo | null>
  compareVersions(scriptName: string, versionA: string, versionB: string): Promise<VersionDiff>
  deleteVersion(scriptName: string, versionId: string): Promise<boolean>
 ...
 */
async function loadDeployerDO(): Promise<new (ctx: MockDOState, env: MockDeployerEnv) => DeployerDOVersionContract> {
...
describe('DeployerDO Version Management', () => {
  ...
  let DeployerDO: new (ctx: MockDOState, env: MockDeployerEnv) => DeployerDOVersionContract
  ...
  describe('getActiveVersion() - Get currently deployed version', () => {
    it('should return the version that is currently active', async () => {
      ...
      const fullInstance = instance as unknown as {
        deploy: (params: { scriptName: string; versionId: string }) => Promise<{ deploymentId: string }>
      }
  ...
  describe('compareVersions() - Diff between versions', () => {
    ...
    it('should throw for non-existent version', async () => {
      ...
      await expect(
        instance.compareVersions('missing-version-worker', v1.id, 'non-existent')
  ...
  describe('deleteVersion() - Remove a version', () => {
    ...
    it('should not allow deleting active version', async () => {
      ...
      await expect(
        instance.deleteVersion('active-delete-worker', version.id)
      ).rejects.toThrow(/cannot delete.*active|currently deployed/i)
    })
  })
  ...
  describe('HTTP endpoints for versions', () => {
    ...
    it('should handle GET /api/scripts/:name/versions/:versionId', async () => {
      ...
      const request = new Request(
        `http://deployer.do/api/scripts/get-single-version-worker/versions/${version.id}`,
        { method: 'GET' }
      )
      const response = await instance.fetch(request)

      expect(response.status).toBe(200)
      const retrieved = await response.json() as VersionInfo
    ...
    it('should handle DELETE /api/scripts/:name/versions/:versionId', async () => {
      ...
      const request = new Request(
        `http://deployer.do/api/scripts/delete-http-version-worker/versions/${version.id}`,
        { method: 'DELETE' }
      )
      const response = await instance.fetch(request)
