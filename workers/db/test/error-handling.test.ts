...
/**
 * Interface definition for DatabaseDO error handling
 */
interface DatabaseDOContract {
...
describe('DatabaseDO Error Handling', () => {
  ...
  describe('Input validation errors', () => {
    ...
    it('should reject null/undefined documents', async () => {
      const instance = new DatabaseDO(ctx, env)
      await expect(instance.create('users', null as never)).rejects.toThrow(/invalid.*document/i)
      await expect(instance.create('users', undefined as never)).rejects.toThrow(/invalid.*document/i)
    })
