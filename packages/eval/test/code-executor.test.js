...
import { createExecutor, createEvaluator, Evaluator, } from '../src/index.js';
describe('CodeExecutor - Workers Compatibility', () => {
    ...
    describe('execute() - Timeout Enforcement', () => {
        ...
        it('should timeout long-running async operations', async () => {
            const result = await executor.execute('await new Promise(r => setTimeout(r, 10000))', {
                timeout: 100,
                allowAsync: true,
            });
            expect(result.success).toBe(false);
        ...
        it('should respect maxTimeout from config', async () => {
            ...
            const result = await limitedExecutor.execute('await new Promise(r => setTimeout(r, 1000))', {
    ...
    describe('execute() - Async Code', () => {
        ...
        it('should execute async code when allowAsync is true', async () => {
            const result = await executor.execute('return await Promise.resolve(42)', { allowAsync: true });
            expect(result.success).toBe(true);
            expect(result.value).toBe(42);
        });
        ...
        it('should reject async code when allowAsync is false', async () => {
            const result = await executor.execute('return await Promise.resolve(42)', { allowAsync: false });
            expect(result.success).toBe(false);
        });
