...
describe('DatabaseDO Query Operations', () => {
    ...
    describe('SQL query() - Direct SQL access', () => {
        it('should execute SELECT queries', async () => {
            ...
            const results = await instance.query('users', 'SELECT * FROM documents WHERE collection = ?', ['users']);
            expect(results).toBeInstanceOf(Array);
        });
        it('should support parameterized queries', async () => {
            ...
            const results = await instance.query('users', 'SELECT * FROM documents WHERE collection = ? AND json_extract(data, "$.name") = ?', ['users', 'Alice']);
            expect(results).toHaveLength(1);
        });
        it('should return typed results', async () => {
            const instance = new DatabaseDO(ctx, env);
            await instance.create('users', { _id: '1', name: 'Test', email: 'test@example.com' });
            const users = await instance.query('users', 'SELECT * FROM documents WHERE collection = ?', ['users']);
