...
import { DrizzleMigrations, SchemaValidator, createMigrations, createSchemaValidator, } from '../src/index.js';
...
describe('Drizzle Migrations Contract', () => {
    ...
    describe('Migration Execution', () => {
        ...
        it('should reject running already applied migration', async () => {
            ...
            await expect(migrations.runSingle('20240101000000_initial')).rejects.toThrow('Migration already applied');
        });
        it('should reject running non-existent migration', async () => {
            const migrations = createMigrations();
            await expect(migrations.runSingle('non_existent_migration')).rejects.toThrow('Migration not found');
        });
    ...
    describe('Migration Rollback', () => {
        ...
        it('should reject rollback to unapplied migration', async () => {
            ...
            await migrations.runSingle('20240101000000_initial');
            await expect(migrations.rollbackTo('20240201000000_add_users')).rejects.toThrow('Target migration not applied');
        });
    });
    describe('Migration Status', () => {
        ...
        it('should include appliedAt for applied migrations', async () => {
            ...
            await migrations.run();
            const status = await migrations.getStatus();
            const appliedMigrations = status.filter((s) => s.applied);
            appliedMigrations.forEach((s) => {
        ...
        it('should not include appliedAt for pending migrations', async () => {
            const migrations = createMigrations();
            const status = await migrations.getStatus();
            const pendingMigrations = status.filter((s) => !s.applied);
            pendingMigrations.forEach((s) => {
    ...
    describe('Migration Table Management', () => {
        it('should create migrations table if not exists', async () => {
            ...
            expect(sql.exec).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS'));
        });
        it('should use custom migrations table name', async () => {
            ...
            await migrations.getStatus();
            expect(sql.exec).toHaveBeenCalledWith(expect.stringContaining('custom_migrations'));
        });
        it('should store migration metadata in table', async () => {
            const migrations = createMigrations();
            await migrations.runSingle('20240101000000_initial');
            // Should have inserted record into migrations table
            expect(sql.exec).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO'), expect.anything());
        });
        it('should remove migration metadata on rollback', async () => {
            ...
            expect(sql.exec).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM'), expect.anything());
        });
    });
});
describe('Schema Validation Contract', () => {
    ...
    describe('Schema Diff', () => {
        ...
        it('should detect added/removed columns', async () => {
            ...
            const diff = await validator.diff(currentSchema, targetSchema);
            expect(diff.some((s) => s.includes('ADD COLUMN'))).toBe(true);
            expect(diff.some((s) => s.includes('DROP COLUMN'))).toBe(true);
        });
