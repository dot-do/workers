...
/**
 * Interface definition for FunctionsDO - this defines the contract
 * The implementation must satisfy this interface
 */
export interface FunctionsDOContract {
  // Core AI primitives (ai-functions compatible)
  generate(prompt: string, options?: GenerateOptions): Promise<GenerateResult>
  list<T>(prompt: string, options?: ListOptions): Promise<T[]>
  extract<T>(text: string, schema: ExtractSchema, options?: ExtractOptions): Promise<T>
  classify(text: string, categories: string[], options?: ClassifyOptions): Promise<ClassifyResult>
  summarize(text: string, options?: SummarizeOptions): Promise<string>
  translate(text: string, targetLanguage: string, options?: TranslateOptions): Promise<string>
  embed(text: string | string[], options?: EmbedOptions): Promise<number[] | number[][]>

  // Function invocation
  invoke(name: string, params: unknown): Promise<unknown>
  register(definition: FunctionDefinition): Promise<void>
  ...
  describe('ai-functions core operations', () => {
    ...
    describe('list()', () => {
      it('should extract a list of items from a prompt', async () => {
        const instance = new FunctionsDO(ctx, env)
        const result = await instance.list<string>('List 5 programming languages')
      ...
      it('should respect maxItems option', async () => {
        const instance = new FunctionsDO(ctx, env)
        const result = await instance.list<string>('List all countries', { maxItems: 3 })
      ...
      it('should support typed results with schema', async () => {
        ...
        const instance = new FunctionsDO(ctx, env)
        const result = await instance.list<Person>('List 3 fictional characters with ages', {
          schema: {
            type: 'array',
            items: {
              type: 'object',
    ...
    describe('extract()', () => {
      it('should extract structured data from text', async () => {
        ...
        const result = await instance.extract<{ name: string; age: number; city?: string }>(text, schema)
      ...
      it('should support strict mode for exact schema matching', async () => {
        ...
        const result = await instance.extract<{ email: string; phone: string }>(text, schema, { strict: true })
    ...
    describe('classify()', () => {
      ...
      it('should support multi-label classification', async () => {
        const instance = new FunctionsDO(ctx, env)
        const result = await instance.classify(
          'Breaking: Tech company announces new AI product',
          ['tech', 'business', 'politics', 'sports'],
          { multiLabel: true }
        )
        expect(result).toHaveProperty('category')
      })
  ...
  describe('HTTP fetch() handler', () => {
    describe('RPC endpoint', () => {
      it('should handle POST /rpc with method call', async () => {
        ...
        const result = await response.json() as { result: GenerateResult }
    ...
    describe('REST API endpoints', () => {
      ...
      it('should handle GET /api/functions to list registered functions', async () => {
        ...
        const response = await instance.fetch(request)
        expect(response.status).toBe(200)
        const data = await response.json() as FunctionDefinition[]
        expect(Array.isArray(data)).toBe(true)
      })
    ...
    describe('HATEOAS discovery', () => {
      ...
      it('should include registered functions in discovery', async () => {
        ...
        const request = new Request('http://functions.do/', { method: 'GET' })
        const response = await instance.fetch(request)
        const data = await response.json() as { discover: { functions: Array<{ name: string }> } }
        expect(data.discover.functions.some((f: { name: string }) => f.name === 'customFunc')).toBe(true)
      })
    })
  })
})
