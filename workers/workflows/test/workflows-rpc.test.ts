...
export interface WorkflowTrigger {
  ...
  /** Event pattern for event triggers (e.g., "$.on.user.created") */
  event?: string
  /** Cron expression for schedule triggers (e.g., "$.every.5.minutes") */
  schedule?: string
...
/**
 * Interface definition for WorkflowsDO - this defines the contract
 * The implementation must satisfy this interface
 */
export interface WorkflowsDOContract {
  ...
  updateWorkflow(workflowId: string, updates: Partial<WorkflowDefinition>): Promise<WorkflowDefinition | null>
  deleteWorkflow(workflowId: string): Promise<boolean>
  listWorkflows(options?: ListOptions): Promise<WorkflowDefinition[]>

  // Workflow Execution
  startWorkflow(workflowId: string, input?: Record<string, unknown>): Promise<WorkflowExecution>
  getExecution(executionId: string): Promise<WorkflowExecution | null>
  listExecutions(workflowId?: string, options?: ListOptions): Promise<WorkflowExecution[]>
  ...
  // Trigger Management
  registerTrigger(workflowId: string, trigger: WorkflowTrigger): Promise<void>
  unregisterTrigger(workflowId: string): Promise<boolean>
  listTriggers(): Promise<Array<{ workflowId: string; trigger: WorkflowTrigger }>>
 ...
 */
async function loadWorkflowsDO(): Promise<new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsDOContract> {
...
describe('WorkflowsDO RPC Interface', () => {
  ...
  let WorkflowsDO: new (ctx: MockDOState, env: MockWorkflowsEnv) => WorkflowsDOContract
    ...
    describe('REST API endpoint', () => {
      ...
      it('should handle DELETE /api/workflows/:id', async () => {
        ...
        const getResponse = await instance.fetch(
          new Request('http://workflows.do/api/workflows/deletable', { method: 'GET' })
        )
