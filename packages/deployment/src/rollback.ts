...
export class DeploymentManager {
  private historyByWorker: Map<string, Deployment[]> = new Map()
   ...
   */
  async getHistory(workerName: string): Promise<DeploymentHistory> {
   ...
   */
  async getCurrentDeployment(workerName: string): Promise<Deployment | undefined> {
    const history = this.historyByWorker.get(workerName) ?? []
   ...
   */
  async rollback(workerName: string): Promise<RollbackResult> {
    const history = this.historyByWorker.get(workerName)
    ...
    const previousDeployments = history.filter(
      d => d.id !== currentDeployment.id && d.status !== 'failed'
    )
   ...
   */
  async rollbackTo(workerName: string, deploymentId: string): Promise<RollbackResult> {
    const history = this.historyByWorker.get(workerName)
   ...
   */
  async canRollback(workerName: string): Promise<boolean> {
    const history = this.historyByWorker.get(workerName)
    ...
    const validPreviousDeployments = history.filter(
      d => d.id !== currentDeployment.id && d.status !== 'failed'
    )
   ...
   */
  async listVersions(workerName: string): Promise<string[]> {
    const history = this.historyByWorker.get(workerName) ?? []
